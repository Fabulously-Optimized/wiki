...
Despite the report button not being there, the framework already exists in it, so it is not any safer than 1.19.1. If you want to stay on an older version, use the latest version for 1.18.2.

### Why not stay on 1.18.2?

You are free to play it for as long as you like (it is safe), but the modpack still prefers to focus on "fixing" the new version(s) rather than keep updating the old one.
